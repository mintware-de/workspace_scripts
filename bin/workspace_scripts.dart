...
Future<void> _run(List<String> arguments) async {
  if (arguments.isEmpty) {
    stdout.writeln('missing script name');
    return;
  }
  ...
  if (script == null) {
    stdout.writeln('script not found');
    return;
  }
...
Config? _loadConfig() {
  ...
  if (!pubspec.existsSync()) {
    stdout.writeln('No pubspec.yaml');
    return null;
  }
  ...
  if (!map.containsKey('workspace')) {
    stdout.writeln('No workspace pubspec.yaml');
    return null;
  }

  if (!map.containsKey('workspace_scripts')) {
    stdout.writeln('No workspace scripts found.');
    return null;
  }
